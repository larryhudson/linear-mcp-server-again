...
server.tool(
  ...
  {
    is_unassigned: z.boolean().optional().describe("Filter for unassigned issues (true) or assigned issues (false)"),
    ...
    limit: z.number().min(1).max(100).optional().default(20).describe("Maximum number of issues to return (default: 20)")
  },
  async ({ is_unassigned, team_identifier, status, is_current_cycle, limit }) => {
    try {
      ...
      if (is_unassigned !== undefined) {
        if (is_unassigned) {
          filter.assignee = { id: { null: true } };
        } else {
          filter.assignee = { id: { null: false } };
        }
      }
